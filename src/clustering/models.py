...
class CrimeClusteringPipeline:
    ...
    def prepare_matrix(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        matrix = (
            df.groupby(["entidad", "bien_juridico_afectado"])["incidencia_delictiva"]
            .sum()
    ...
    def evaluate_k_options(self, max_k: int = 10) -> dict[str, list[float]]:
        ...
        if self.X_scaled is None:
            raise RuntimeError(
                "Primero debes ejecutar `fit_transform()` sobre los datos."
            )
    ...
    def save(self, df: pd.DataFrame) -> None:
        ...
        df_segmentado = pd.DataFrame(
            {"entidad": df["entidad"], "cluster_bienes_juridicos": df["cluster"]}
        )
